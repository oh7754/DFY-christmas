...
// ✅ CSS background-image 안전 세터 (괄호/공백 포함 URL 깨짐 방지)
...
async function uploadAndRegister(file) {
  ...
  const safeName = (processedFile.name || "image.jpg").replace(/[^\\w.\\-]+/g, "_");

  const filePath = `uploads/${currentUser.uid}/${Date.now()}_${safeName}`;
...
function showWishPanel(data) {
  ...
  setBgImage(wishViewImageEl, data.url);
...
// 사이드 패널: 캔버스나 빈 공간 클릭 시 닫기
document.addEventListener("click", (e) => {
  if (!sidePanel || !menuToggle || !topAccount) return;

  const target = e.target;

  // 사이드 패널/헤더(계정, 햄버거 버튼) 영역 안이면 무시
  const clickInsideSide =
    sidePanel.contains(target) ||
    topAccount.contains(target) ||
